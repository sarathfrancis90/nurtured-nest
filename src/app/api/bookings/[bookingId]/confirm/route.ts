...
export async function POST(request: NextRequest, { params }: { params: { bookingId: string } }) {
  ...
  try {
    ...
    const booking = await updateBookingStatus(params.bookingId, parsed, 'confirm');
    ...
    const code = (err as { code?: string }).code ?? 'confirm_error';
    const status = (err as { status?: number }).status ?? 400;
    return NextResponse.json(errorOut(requestId, code, (err as Error).message || 'Unable to confirm booking'), { status });
  }
}
